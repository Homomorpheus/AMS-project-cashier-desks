...
def plot_queue_length_statistics(total_queue_length_data, simulation_end_time):
    # plotting
    ...
    mean = timeseries_tools.time_series_mean(total_queue_length_data, x)
    ax[0].plot(x, mean, label="Total queue length mean")
    # ax[0].step(x, mean, label="Queue length mean", where="post")

    # mean +- standard deviation
    std_dev = timeseries_tools.time_series_std_deviation(total_queue_length_data, x)
    ax[0].plot(x, mean - std_dev, label="mean - std deviation")
    ax[0].plot(x, mean + std_dev, label="mean + std deviation")

    # median
    mean = timeseries_tools.time_series_quantile(total_queue_length_data, x, 0.5)
    ax[1].plot(x, mean, label="Queue length median")

    # 1. quartile
    quartile_1 = timeseries_tools.time_series_quantile(total_queue_length_data, x, 0.25)
    ax[1].plot(x, quartile_1, label="1. quartile")

    # 3. quartile
    quartile_2 = timeseries_tools.time_series_quantile(total_queue_length_data, x, 0.75)
...
if __name__=="__main__":
    ...
    simulation_results = list(map(lambda seed: simulate_multiqueue(seed, simulation_end_time, service_time, interarr_time, amount_cashiers=k), seeds))

    # plot queue length
    total_queue_length_data = [sum([result[0][i] for i in range(k)]) for result in simulation_results]
    plot_queue_length_statistics(total_queue_length_data, simulation_end_time)
    ...
    for i in range(k):
        customer_waiting_times = list(itertools.chain.from_iterable(result[2][i] for result in simulation_results))
        print(f"Customer waiting time in Queue {i+1}:")
